...
const server = http.createServer((req, res) => {
	...
	const fullUrl = "http://localhost" + req.url; // base обязателен
		...
		for (let i = 0; i < old_j.length; i++) {
			if (tools.compareSystems(old_j[i], data)) {
				if (action == "Delete") {
					...
					console.log("758: The " + prop + " was: " + old_j[i][prop]);
	...
	checkOfficers() {
		...
		for (let group of groups) {
			...
			that.requestZkb(url1, syst, argument, function (id, date) {
				...
				if (that.data[group] != undefined && that.data[group][0] != id) {
					(async () => { await sendMessageToChannel(url); })();
				}
...
class swagger{
	...
	******************************************************************/
	...
	|=|	обновляем имя перса
	******************************************************************/
	updateCharName(token, charID, charLoc,crestDB,j){
			var host = 'esi.evetech.net';
...
function webhooksSend(txt, id, inf, that) {
	...
	if (data != "") {
		...
		//Telegram:
		(async () => {await sendMessageToChannel(data);})();
	}
}
/*****************************************************************
	вспомогательные функции будут здесь
